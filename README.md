...
| <a name="input_manual_servers"></a> [manual\_servers](#input\_manual\_servers) | The DNS servers to use for the VPC, replacing any existing servers. All the DNS servers must either: have a unique zone\_affinity, or not have a zone\_affinity. | <pre>list(object({<br>    address       = string<br>    zone_affinity = optional(string)<br>  }))</pre> | `[]` | no |
